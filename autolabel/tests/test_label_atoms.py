...
def test_filter_by_confidence():
    ...
    # With threshold 0.7, only bottle (0.9) should pass
    filtered = filter_by_confidence(detections, 0.7)
    assert len(filtered) == 1
    assert filtered[0]["class_name"] == "bottle"

    # With threshold 0.5, bottle (0.9) and can (0.6) should pass
...
def test_map_class_names():
    """Map class names to class IDs using class_map."""
    # Detection with class_name that exists in class_map
    detections = [
        {
            "class_id": 0,
            "class_name": "potato",
    ...
    # map_class_names maps class_name to the value in class_map (potato -> 0)
    assert mapped[0]["class_id"] == 0
    # The class_name is replaced with the mapped value from class_map
    assert mapped[0]["class_name"] == 0


def test_nms():
    """Non-maximum suppression should keep highest confidence detection."""
    detections = [
        {
            "class_id": 0,
            "confidence": 0.9,
...
def test_nms_no_overlap():
    """Non-maximum suppression with no overlapping boxes."""
    detections = [
        {
            "class_id": 0,
            "confidence": 0.9,
        ...
        },
        {
            "class_id": 1,
            "confidence": 0.8,
            "bbox": {"x1": 60, "y1": 60, "x2": 110, "y2": 110},
        },
    ]
